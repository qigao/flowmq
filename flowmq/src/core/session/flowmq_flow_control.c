...
int flowmq_flow_control_init_local(flowmq_flow_control_t *state,
  ...
  if (state->local_initialized || state->remote_initialized) return SALTS_EALREADY;
...
int flowmq_flow_control_make_settings(const flowmq_flow_control_t *state,
  ...
  if (!state || !settings || max_frame_size == 0u) return SALTS_EINVAL;
  if (!state->local_initialized) return SALTS_EBUSY;
...
int flowmq_flow_control_apply_settings(flowmq_flow_control_t *state,
  ...
  int status;
  if (!state || !settings) return SALTS_EINVAL;
  if (!state->local_initialized) return SALTS_EBUSY;
  if (state->remote_initialized) return SALTS_EPROTO;
...
int flowmq_flow_control_send_check(const flowmq_flow_control_t *state,
                                   size_t payload_size) {
  if (!state) return SALTS_EINVAL;
  if (!state->remote_initialized) return SALTS_EBUSY;
  if ((uint64_t)payload_size > state->remote_max_frame_size) return SALTS_EMSGSIZE;
...
int flowmq_flow_control_next_update(const flowmq_flow_control_t *state,
  ...
  if (!state || !update) return SALTS_EINVAL;
  if (!state->local_initialized) return SALTS_EBUSY;
  if (!state->update_pending) return FLOWMQ_FLOW_CONTROL_NO_UPDATE;
  if (state->consumed_data < state->advertised_consumed_data) return SALTS_EPROTO;
...
int flowmq_flow_control_mark_update_sent(
  ...
  if (!state || !update) return SALTS_EINVAL;
  if (!state->local_initialized || !state->update_pending) return SALTS_EBUSY;
  if (state->consumed_data > UINT64_MAX - state->receive_window)
    return SALTS_ERANGE;
