...
void flowmq_subscription_set_clear(flowmq_subscription_set_t *subscriptions) {
  ...
  for (size_t i = 0u; i < vec_size(&subscriptions->entries); ++i) {
    flowmq_subscription_t *subscription =
        (flowmq_subscription_t *)vec_at(&subscriptions->entries, i);
    if (subscription) tstr_freep(&subscription->topic);
  }
  vec_clear(&subscriptions->entries);
}
...
void flowmq_subscription_set_destroy(flowmq_subscription_set_t *subscriptions) {
  ...
  if (subscriptions->initialized) {
    ...
    vec_destroy(&subscriptions->entries);
  }
  ...
  *changed = 0;
  for (size_t i = 0u; i < vec_size(&subscriptions->entries); ++i) {
    ...
    if (subscribe) {
      ...
      int rc = vec_swap_remove(&subscriptions->entries, i, &removed);
      if (rc != STL_OK) return flowmq_stl_error((stl_status)rc);
...
size_t flowmq_subscription_set_count(const flowmq_subscription_set_t *subscriptions) {
  return subscriptions && subscriptions->initialized ? vec_size(&subscriptions->entries) : 0u;
}
...
flowmq_subscription_set_at(const flowmq_subscription_set_t *subscriptions, size_t index) {
  ...
  return (const flowmq_subscription_t *)vec_at_const(&subscriptions->entries, index);
}
