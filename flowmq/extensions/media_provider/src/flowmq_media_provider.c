...
static int flowmq_media_provider_text(const char *value, size_t maximum, int required) {
  size_t size;
...
flowmq_media_provider_envelope_validate(const flowmq_media_provider_envelope_view_t *envelope,
  ...
  int rc;
  if (!envelope || flowmq_media_provider_limits_validate(limits) != TURBO_OK) return TURBO_EINVAL;
  if (envelope->schema_version != FLOWMQ_MEDIA_PROVIDER_SCHEMA_VERSION) return TURBO_ENOTSUP;
...
#define FLOWMQ_VALIDATE_ENVELOPE_TEXT(member, maximum, required)                                   \
  ...
  FLOWMQ_VALIDATE_ENVELOPE_TEXT(session_id, limits->maximum_id_bytes, require_session);
...
#define FLOWMQ_ENVELOPE(message)                                                                   \
  (flowmq_media_provider_envelope_view_t) {                                                        \
    (message)->schema_version, (message)->message_kind, (message)->message_id,                     \
        (message)->correlation_id, (message)->causation_id, (message)->tenant_id,                  \
        (message)->provider_id, (message)->session_id, (message)->partition_key,                   \
        (message)->producer_id, (message)->created_at, (message)->deadline_at                      \
  }
...
int flowmq_media_provider_peek_kind(const void *encoded, size_t encoded_size,
  ...
  if (!ProviderCommandV1_view_bind(&view, encoded, encoded_size)) return TURBO_EPROTO;
...
int flowmq_media_provider_validate_command(const ProviderCommandV1_t *message,
                                           const flowmq_media_provider_limits_t *limits) {
  ...
  if ((rc = flowmq_media_provider_text(message->command_id, limits->maximum_id_bytes, 1)) !=
      ...
      (rc = flowmq_media_provider_text(message->worker_id, limits->maximum_id_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->semantic_fingerprint, limits->maximum_id_bytes,
                                       1)) != TURBO_OK ||
      (rc = flowmq_media_provider_text(message->payload_json, limits->maximum_payload_bytes, 1)) !=
          TURBO_OK)
    return rc;
...
int flowmq_media_provider_validate_receipt(const ProviderReceiptV1_t *message,
                                           const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t epoch;
  int rc;
...
int flowmq_media_provider_validate_completion(const ProviderCompletionV1_t *message,
                                              const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t epoch;
  ...
  if ((rc = flowmq_media_provider_text(message->command_id, limits->maximum_id_bytes, 1)) !=
      ...
      (rc = flowmq_media_provider_text(message->event_type, limits->maximum_type_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->completed_at, limits->maximum_timestamp_bytes,
                                       1)) != TURBO_OK ||
      (rc = flowmq_media_provider_text(message->completed_at_unix_ms,
                                       limits->maximum_timestamp_bytes, 1)) != TURBO_OK ||
      (rc = flowmq_media_provider_text(message->result_json, limits->maximum_payload_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->error_code, limits->maximum_type_bytes, 0)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->error_message, limits->maximum_error_bytes, 0)) !=
  ...
  rc = flowmq_media_provider_parse_u64(message->completed_at_unix_ms, &completed_at_unix_ms);
  return rc == TURBO_OK && completed_at_unix_ms == 0u ? TURBO_EPROTO : rc;
}

int flowmq_media_provider_validate_completion_ack(const ProviderCompletionAckV1_t *message,
                                                  const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t epoch;
  uint64_t sequence;
  ...
  if (rc != TURBO_OK) return rc;
  if (!ProviderCompletionAckDisposition_is_valid(message->disposition)) return TURBO_EPROTO;
  if ((rc = flowmq_media_provider_text(message->command_id, limits->maximum_id_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->error_code, limits->maximum_type_bytes, 0)) !=
          TURBO_OK ||
...
int flowmq_media_provider_validate_event(const ProviderEventV1_t *message,
                                         const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t sequence;
  int rc;
  ...
  if ((rc = flowmq_media_provider_text(message->event_id, limits->maximum_id_bytes, 1)) !=
      ...
      (rc = flowmq_media_provider_text(message->aggregate_id, limits->maximum_id_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->occurred_at, limits->maximum_timestamp_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->payload_json, limits->maximum_payload_bytes, 1)) !=
          TURBO_OK)
    return rc;
...
int flowmq_media_provider_validate_event_ack(const ProviderEventAckV1_t *message,
                                             const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t sequence;
  int rc;
...
int flowmq_media_provider_validate_query(const ProviderQueryV1_t *message,
                                         const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t revision;
  int rc;
...
int flowmq_media_provider_validate_observation(const ProviderObservationV1_t *message,
                                               const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t revision;
  int rc;
...
int flowmq_media_provider_validate_call_offer(const ProviderCallOfferV1_t *message,
                                              const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t generation;
  int rc;
  ...
  if ((rc = flowmq_media_provider_text(message->ingress_event_id, limits->maximum_id_bytes, 1)) !=
      ...
      (rc = flowmq_media_provider_text(message->transport, limits->maximum_type_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->source, limits->maximum_id_bytes, 1)) != TURBO_OK ||
...
int flowmq_media_provider_validate_session_bound(const ProviderSessionBoundV1_t *message,
                                                 const flowmq_media_provider_limits_t *limits) {
  flowmq_media_provider_envelope_view_t envelope;
  uint64_t generation;
  int rc;
  if (!message) return TURBO_EINVAL;
  envelope = FLOWMQ_ENVELOPE(message);
  rc = flowmq_media_provider_envelope_validate(&envelope, limits, message->accepted != 0,
  ...
  if ((rc = flowmq_media_provider_text(message->ingress_event_id, limits->maximum_id_bytes, 1)) !=
      ...
      (rc = flowmq_media_provider_text(message->call_id, limits->maximum_id_bytes, 1)) !=
          TURBO_OK ||
      (rc = flowmq_media_provider_text(message->bound_session_id, limits->maximum_id_bytes,
