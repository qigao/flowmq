...
spec("flowmq_reconnect") {
  it("grows bounded backoff and resets only after success") {
    ...
    check_equal(flowmq_reconnect_init(&reconnect, 20u, 80u, 17u), SALTS_OK);
  ...
  it("keeps a fixed interval when max is zero") {
    ...
    check_equal(flowmq_reconnect_init(&reconnect, 20u, 0u, 17u), SALTS_OK);
  ...
  it("represents an immediate interval without disabling the policy") {
    ...
    check_equal(flowmq_reconnect_init(&reconnect, 0u, 0u, 0u), SALTS_OK);
    check_equal(flowmq_reconnect_next(&reconnect, &delay_ms), SALTS_OK);
    check_equal(delay_ms, 0u);
    check_equal(flowmq_reconnect_init(&reconnect, 100u, 50u, 0u), SALTS_EINVAL);
  }
}
