...
> 本文描述的是历史 TurboFlow Application facade，不属于独立 FlowMQ
> 构建。当前可构建的 standalone 示例见
> [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md#4-可直接运行的示例)。

以下旧目标与 YAML host 不再由 `BUILD_EXAMPLES` 生成：

```powershell
flowmq_zmq_style_pub_sub
flowmq_zmq_style_req_rep
flowmq_zmq_style_router_dealer
flowmq_yaml_endpoint
```
