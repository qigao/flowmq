...
cmake --build --preset win-release-user --target `
  flowmq_protocol_roundtrip `
  flowmq_segmented_frame `
  flowmq_security_hello `
  flowmq_router_dealer
```

运行：

```powershell
build\Msvc-Release\bin\flowmq_protocol_roundtrip.exe
build\Msvc-Release\bin\flowmq_segmented_frame.exe
build\Msvc-Release\bin\flowmq_security_hello.exe
build\Msvc-Release\bin\flowmq_router_dealer.exe
```

完整、可编译源码：

- [FMQ v3 frame round-trip](examples/protocol_roundtrip.c)
- [Scatter/gather frame encoding](examples/segmented_frame.c)
- [Security HELLO encoding](examples/security_hello.c)
- [TCP ROUTER/DEALER round-trip](examples/router_dealer.c)
