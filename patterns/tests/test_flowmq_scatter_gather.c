...
spec("flowmq_scatter_gather") {
  ...
  it("creates scatter session with ALL policy") {
    ...
    check_equal(flowmq_scatter_gather_create_session(&manager, 3, FLOWMQ_GATHER_ALL,
  ...
  it("completes with QUORUM policy") {
    ...
    check_equal(flowmq_scatter_gather_create_session(&manager, 5, FLOWMQ_GATHER_QUORUM,
                                                      now_ns + 5000000000ULL, NULL,
                                                      &correlation_id),
                 TURBO_OK);
  ...
  it("handles error responses") {
    ...
    check_equal(flowmq_scatter_gather_create_session(&manager, 2, FLOWMQ_GATHER_ALL,
                                                      now_ns + 5000000000ULL, NULL,
                                                      &correlation_id),
                 TURBO_OK);
  ...
  it("uses manager default timeout when timeout_ms is zero") {
    ...
    check_equal(flowmq_scatter_gather_create_session(&manager, 2, FLOWMQ_GATHER_ALL, 0ULL, NULL,
                                                      &correlation_id),
                 TURBO_OK);
  ...
  it("creates session with relative timeout in milliseconds explicitly") {
    ...
    uint64_t correlation_id = 0;
    check_equal(flowmq_scatter_gather_create_session_ms(&manager, 1, FLOWMQ_GATHER_ALL, 2u,
                                                       NULL, &correlation_id),
                 TURBO_OK);
  ...
  it("rejects overflow for relative timeout conversion in milliseconds") {
    flowmq_scatter_gather_manager_t manager;
    check_equal(flowmq_scatter_gather_manager_init(&manager, 10, 50), TURBO_OK);
    check_equal(flowmq_scatter_gather_create_session_ms(&manager, 1, FLOWMQ_GATHER_ALL,
  ...
  it("enforces capacity limit") {
    ...
    check_equal(flowmq_scatter_gather_create_session(&manager, 1, FLOWMQ_GATHER_ALL,
