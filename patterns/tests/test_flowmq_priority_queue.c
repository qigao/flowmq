...
TEST_SUITE(flowmq_priority_queue) {
  it("initializes and destroys priority queue") {
    ...
    check_equal(flowmq_priority_queue_init(&pq, 1000, 10000), TURBO_OK);
    
    uint64_t total_messages = 0, total_bytes = 0;
    uint8_t max_priority = 0;
    flowmq_priority_queue_stats(&pq, &total_messages, &total_bytes, &max_priority);
    check_equal(total_messages, 0);
    check_equal(total_bytes, 0);
    check_equal(flowmq_priority_queue_is_empty(&pq), 1);
  ...
  it("enqueues and dequeues single message") {
    flowmq_priority_queue_t pq;
    check_equal(flowmq_priority_queue_init(&pq, 100, 10000), TURBO_OK);
    ...
    check_equal(flowmq_priority_queue_enqueue(&pq, 128, &payload, &topic, 1000000000ULL),
                 TURBO_OK);
    
    check_equal(flowmq_priority_queue_is_empty(&pq), 0);
    
    flowmq_priority_message_t msg = {0};
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 128);
    check_equal(msg.payload, "hello");
  ...
  it("dequeues messages in priority order") {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      check_equal(flowmq_priority_queue_enqueue(&pq, priorities[i], &payload, &topic,
                                                 1000000000ULL),
                   TURBO_OK);
    }
    ...
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 200);
    check_equal(msg.payload, "msg1");
    flowmq_priority_message_cleanup(&msg);
    
    /* Second: priority 200, msg3 */
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 200);
    check_equal(msg.payload, "msg3");
    flowmq_priority_message_cleanup(&msg);
    
    /* Third: priority 150, msg4 */
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 150);
    check_equal(msg.payload, "msg4");
    flowmq_priority_message_cleanup(&msg);
    
    /* Fourth: priority 100, msg0 */
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 100);
    check_equal(msg.payload, "msg0");
    flowmq_priority_message_cleanup(&msg);
    
    /* Fifth: priority 50, msg2 */
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_OK);
    check_equal(msg.priority, 50);
    check_equal(msg.payload, "msg2");
    flowmq_priority_message_cleanup(&msg);
  ...
  it("maintains FIFO order within same priority") {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      check_equal(msg.payload, expected);
      flowmq_priority_message_cleanup(&msg);
    }
  ...
  it("enforces message count capacity limit") {
    flowmq_priority_queue_t pq;
    check_equal(flowmq_priority_queue_init(&pq, 3, 100000), TURBO_OK);  /* Max 3 messages */
  ...
  it("enforces byte capacity limit") {
    flowmq_priority_queue_t pq;
    check_equal(flowmq_priority_queue_init(&pq, 1000, 20), TURBO_OK);  /* Max 20 bytes */
  ...
  it("peeks without removing message") {
    ...
    topic = tstr_dup("test");
    check_equal(flowmq_priority_queue_enqueue(&pq, 200, &payload, &topic, 1000000000ULL),
                 TURBO_OK);
    
    /* Peek multiple times */
    const flowmq_priority_message_t *msg = NULL;
    check_equal(flowmq_priority_queue_peek(&pq, &msg), TURBO_OK);
    check_equal(msg->priority, 200);
    check_equal(msg->payload, "peek-test");
    
    check_equal(flowmq_priority_queue_peek(&pq, &msg), TURBO_OK);
    check_equal(msg->priority, 200);  /* Still there */
  ...
  it("returns error when dequeuing from empty queue") {
    ...
    flowmq_priority_message_t msg = {0};
    check_equal(flowmq_priority_queue_dequeue(&pq, &msg), TURBO_ENOENT);
    
    const flowmq_priority_message_t *peek_msg = NULL;
    check_equal(flowmq_priority_queue_peek(&pq, &peek_msg), TURBO_ENOENT);
  ...
  it("updates max_priority correctly") {
    ...
    tstr p1 = NULL, t1 = NULL;
    p1 = tstr_dup("msg1");
    t1 = tstr_dup("test");
    check_equal(flowmq_priority_queue_enqueue(&pq, 100, &p1, &t1, 1000000000ULL), TURBO_OK);
    flowmq_priority_queue_stats(&pq, NULL, NULL, &max_priority);
    check_equal(max_priority, 100);
    
    /* Enqueue at priority 200 */
    tstr p2 = NULL, t2 = NULL;
    p2 = tstr_dup("msg2");
    t2 = tstr_dup("test");
    check_equal(flowmq_priority_queue_enqueue(&pq, 200, &p2, &t2, 1000000000ULL), TURBO_OK);
    flowmq_priority_queue_stats(&pq, NULL, NULL, &max_priority);
    check_equal(max_priority, 200);
  ...
  it("counts messages at specific priority") {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      payload = tstr_dup("data");
      topic = tstr_dup("test");
      check_equal(flowmq_priority_queue_enqueue(&pq, 100, &payload, &topic, 1000000000ULL),
                   TURBO_OK);
    }
    ...
    check_equal(flowmq_priority_queue_count_at_priority(&pq, 100), 3);
    check_equal(flowmq_priority_queue_count_at_priority(&pq, 200), 2);
    check_equal(flowmq_priority_queue_count_at_priority(&pq, 50), 0);
