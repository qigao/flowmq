 ...
 *
 * State machine:
 *   FLOWMQ_SAGA_RUNTIME_PENDING → FLOWMQ_SAGA_RUNTIME_EXECUTING → FLOWMQ_SAGA_RUNTIME_COMMITTED
 * (all steps succeed) → FLOWMQ_SAGA_RUNTIME_COMPENSATING (step failed) →
...
int flowmq_saga_coordinator_init(flowmq_saga_coordinator_t *coordinator, uint32_t max_transactions);
...
int flowmq_saga_coordinator_create(flowmq_saga_coordinator_t *coordinator, uint32_t num_steps,
                                   uint64_t deadline_ns, void *user_context, tstr *out_saga_id);

/* Add step to SAGA transaction (must be in PENDING state) */
int flowmq_saga_coordinator_add_step(flowmq_saga_coordinator_t *coordinator, const tstr *saga_id,
                                     const tstr *service_name, const tstr *transaction_payload,
                                     const tstr *compensation_payload);

/* Start SAGA execution (move from PENDING → EXECUTING) */
int flowmq_saga_coordinator_start(flowmq_saga_coordinator_t *coordinator, const tstr *saga_id);

/* Record step execution result */
int flowmq_saga_coordinator_record_step_result(flowmq_saga_coordinator_t *coordinator,
                                               const tstr *saga_id, uint32_t step_id, int success,
                                               int error_code, uint64_t timestamp_ns);

/* Trigger compensation for failed SAGA */
int flowmq_saga_coordinator_compensate(flowmq_saga_coordinator_t *coordinator, const tstr *saga_id);

/* Record compensation result */
int flowmq_saga_coordinator_record_compensation(flowmq_saga_coordinator_t *coordinator,
                                                const tstr *saga_id, uint32_t step_id,
...
int flowmq_saga_coordinator_peek_compensation(flowmq_saga_coordinator_t *coordinator,
                                              const tstr *saga_id, uint32_t *out_step_id,
                                              tstr *out_service_name,
                                              tstr *out_compensation_payload);

/* Get SAGA transaction state */
int flowmq_saga_coordinator_get_state(flowmq_saga_coordinator_t *coordinator, const tstr *saga_id,
...
int flowmq_saga_coordinator_abort(flowmq_saga_coordinator_t *coordinator, const tstr *saga_id);

/* Get statistics */
void flowmq_saga_coordinator_stats(flowmq_saga_coordinator_t *coordinator, uint32_t *out_active,
