...
#define FLOWMQ_STREAM_MAX_PARTITIONS FLOWMQ_ESB_MAX_STREAM_PARTITIONS
#define FLOWMQ_STREAM_MAX_CONSUMER_GROUPS FLOWMQ_ESB_MAX_STREAM_CONSUMER_GROUPS
#define FLOWMQ_STREAM_MAX_GROUP_MEMBERS FLOWMQ_ESB_MAX_STREAM_GROUP_MEMBERS
...
typedef char flowmq_stream_partition_runtime_limits_check
    [(FLOWMQ_STREAM_MAX_PARTITIONS == FLOWMQ_ESB_MAX_STREAM_PARTITIONS &&
      FLOWMQ_STREAM_MAX_CONSUMER_GROUPS == FLOWMQ_ESB_MAX_STREAM_CONSUMER_GROUPS &&
      FLOWMQ_STREAM_MAX_GROUP_MEMBERS == FLOWMQ_ESB_MAX_STREAM_GROUP_MEMBERS)
         ? 1
         : -1];
 ...
 */
int flowmq_stream_topic_init(flowmq_stream_topic_t *topic, const char *topic_name,
                             uint32_t partition_count, uint32_t max_messages_per_partition,
                             uint64_t max_bytes_per_partition, uint64_t retention_ms);
 ...
 */
int flowmq_stream_topic_publish(flowmq_stream_topic_t *topic, uint32_t partition_id, tstr *payload,
                                tstr *topic_str, uint64_t timestamp_ns, uint64_t *out_offset);
 ...
 */
int flowmq_stream_topic_fetch(flowmq_stream_topic_t *topic, uint32_t partition_id,
                              uint64_t start_offset, uint32_t max_messages,
                              const flowmq_stream_message_t **out_messages, uint32_t *out_count);
 ...
 */
int flowmq_stream_topic_get_consumer_group(flowmq_stream_topic_t *topic, const char *group_id,
 ...
 */
int flowmq_stream_consumer_group_join(flowmq_stream_consumer_group_t *group, const char *member_id,
                                      uint64_t now_ns);
...
int flowmq_stream_consumer_group_commit(flowmq_stream_consumer_group_t *group,
                                        uint32_t partition_id, uint64_t offset);
...
int flowmq_stream_consumer_group_get_offset(const flowmq_stream_consumer_group_t *group,
                                            uint32_t partition_id, uint64_t *out_offset);
...
void flowmq_stream_partition_stats(const flowmq_stream_partition_t *partition,
                                   uint32_t *message_count, uint64_t *bytes,
                                   uint64_t *low_watermark, uint64_t *high_watermark);
